##Dissertation: visualizing pest data
...
p1<-ggplot(data = data_sum, aes(x = Week, y = n)) + geom_line() +scale_x_date(date_labels = "%Y") + labs(x="Year", y = "Number of Requests", title = "311 Complaints for Rodents/Pests, 2016 - 2021") + #add dashed lines for move in weeks
...
p3<-ggplot(data = sum_16, aes(x = Week, y = n)) + geom_line() +scale_x_date(date_labels = "%m") + labs(x="Months", y = "Number of Requests", title = "311 Complaints for Rodents/Pests (2016)") #x-axis 12 mo showing as 1
p3+geom_vline(xintercept = as.numeric(as.Date("2016-08-28")), linetype = 3)
p3<-ggplotly(p3)
p3

#move in week numbers, same as #s Haddidan presented
moveinpest<-subset(data, data$open_dt_ymd > "2016-08-28" & data$open_dt_ymd < "2016-09-05")
table(moveinpest$type)
